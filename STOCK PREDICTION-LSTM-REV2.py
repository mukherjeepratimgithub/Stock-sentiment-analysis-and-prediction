...
# Replace NaN values with zeros in the 'Sentiment_Score' column
...
def preprocess_data(merged_data):
    # Drop irrelevant columns and keep only 'Adj Close', 'Sentiment_Score'
    merged_data = merged_data[['Adj Close','Sentiment_Score']]
...
# Make predictions
predictions_sentiment = model.predict(X_test)

# Reshape predictions_sentiment to match the expected shape
predictions_sentiment = predictions_sentiment.reshape(-1, 1)

# Inverse transform predictions and actual values
predictions_sentiment = scaler.inverse_transform(np.concatenate((predictions_sentiment, np.zeros_like(predictions_sentiment)), axis=1))
Y_test = scaler.inverse_transform(Y_test)

# Calculate RMSE
rmse = np.sqrt(mean_squared_error(Y_test, predictions_sentiment))
print("Root Mean Squared Error (RMSE):", rmse)
...
plt.plot(predictions_sentiment, label='Predicted', color='red')
